...
#MAke home menu screen
home = pygame.transform.scale( pygame.image.load(os.path.join("HOME1.png")), (900,600))
...
FPS = 100
BGcolour = "#5C5E59"
#imports images for the game
image_numbers = []
for i in range(1,9):
    image_numbers.append(
        pygame.transform.scale(
            pygame.image.load(os.path.join(f"N-{i}.jpeg")), (Tile,Tile)
        )
    )
...
# ================================================================
#  FUNCTIONING PART (GAME LOGIC, CLASSES, AND MAIN LOOP)
# ================================================================
#As the name suggest
class Game :
    def __init__(self):
    ...
    def restart_menu(self):
        ...
        win = pygame.transform.scale(pygame.image.load(os.path.join("win.png")), (Width,Width//4))
        lose = pygame.transform.scale(pygame.image.load(os.path.join("lose.png")), (Width,Width//4))
        one = True
        while one:
            if self.won():
                self.screen.blit(win,(0,Height//2-70))
            else:
                self.screen.blit(lose,(0,Height//2-70))
...
class Tiles:
    ...
    def draw(self,surface):
        #this function draws on the surface
        if not self.flagged and self.known:#draws unflagged and known then draw tile's image
            surface.blit(self.image,(self.x , self.y))
        elif self.flagged and not self.known:#those which are flagged and not known then draw flag
            surface.blit(image_Flag,(self.x,self.y))
        elif not self.known:#for unkown draw unknown
            surface.blit(image_Unknown,(self.x,self.y))


class Board:
    def __init__(self):
        self.surface = pygame.Surface((Width,Height))  #A pygame surface is like a drawing board
        self.board_list = [[Tiles(row,col,image_empty,"U") for col in range(cols)] for row in range(rows)]#acts as our memory
        self.place_mines()#randomly place Minescount number of mines
        self.clues()#sets image of numbered mines to no. of surrounding mines
        self.dug = []#list of duggen up tiles

    def draw(self,screen):
        #draw individual tiles onto surface upon call and then blit the full surface onto screen
    ...
    def clues(self):
        ...
        for row in range(rows):
            for col in range(cols):
                if self.board_list[row][col].type != "M":#if the tile is not a mine count number of adjacent mines
